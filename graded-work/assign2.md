...
As implied above, data for the app will be generated by you (the student) as you work on the app. In addition, the professors will publish a web API that will aggregate the data generated by all students. More info about that will be published soon. 

The database will include at these collections:
* terms in English 
  * embedded collection, definitions in English
* terms in other languages 
  * embedded collection, definitions in other languages

As suggested by this list of collections, a *term* could have zero or more *definitions*. This idea supports alternative or evolving definitions over time. It is implemented as a one-to-many association or relationship that uses a MongoDB technique that "embeds" a document inside another.  

Each collection will have the same data fields:

![schema](media/a2-2020-schemas.png)

In addition, an item in the English collection will be associated or related, in a one-to-many manner, with zero or more items in the non-English collection. It is implemented using a MongoDB technique that stores an object identifier "reference" to another document. 

Soon, you will learn about associated data in a NoSQL data store (like our MongoDB), and these different ways to represent the associations. 
